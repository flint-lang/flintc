...
class Matcher {
  ...
  private:
    ...
    static const inline std::unordered_map<Token, PatternPtr> token_patterns = {
        ...
        {TOK_CHAR, std::make_shared<TokenTypeMatcher>(TOK_CHAR)},
  ...
  public:
    ...
    static const inline PatternPtr type_prim = one_of({
        ...
        token(TOK_F64), token(TOK_FLINT), token(TOK_STR), token(TOK_CHAR), token(TOK_BOOL) //
    });
    ...
    static const inline PatternPtr until_right_bracket =
        balanced_match_until(token(TOK_LEFT_PAREN), token(TOK_RIGHT_BRACKET), token(TOK_RIGHT_PAREN), 1);
    static const inline PatternPtr until_comma = balanced_match_until(token(TOK_LEFT_PAREN), token(TOK_COMMA), token(TOK_RIGHT_PAREN), 1);
