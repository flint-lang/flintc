    ...
    std::unique_ptr<ExpressionNode> clone(const unsigned int scope_id) const override {
        ...
        for (const auto &content : string_content) {
            if (std::holds_alternative<std::unique_ptr<ExpressionNode>>(content)) {
                std::unique_ptr<ExpressionNode> cloned_value = std::get<std::unique_ptr<ExpressionNode>>(content)->clone(scope_id);
                string_content_clone.emplace_back(std::move(cloned_value));
            } else if (std::holds_alternative<std::unique_ptr<LiteralNode>>(content)) {
                std::unique_ptr<ExpressionNode> expr_clone = std::get<std::unique_ptr<LiteralNode>>(content)->clone(scope_id);
