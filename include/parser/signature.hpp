...
namespace Signature {
    ...
    const signature literal = {"((", TOK_STR_VALUE, ")|(", TOK_INT_VALUE, ")|(", TOK_FLINT_VALUE, ")|(", TOK_CHAR_VALUE, ")|(", TOK_TRUE,
        ")|(", TOK_FALSE, "))"};
    const signature type = combine({{"("}, type_prim, {"|(", TOK_IDENTIFIER, "))"}});
    ...
    const signature function_definition =
        combine({{"(", TOK_ALIGNED, ")?", "(", TOK_CONST, ")?", TOK_DEF, TOK_IDENTIFIER, TOK_LEFT_PAREN, "("}, args,
    ...
    const signature data_definition = {"((", TOK_SHARED, ")|(", TOK_IMMUTABLE, "))?(", TOK_ALIGNED, ")?", TOK_DATA, TOK_IDENTIFIER,
        TOK_COLON};
    const signature func_definition =
        combine({{TOK_FUNC, TOK_IDENTIFIER, "(", TOK_REQUIRES, TOK_LEFT_PAREN}, no_prim_args, {TOK_RIGHT_PAREN, ")?", TOK_COLON}});
    ...
    // --- ENTITY DEFINITION ---
    const signature entity_definition =
        combine({{TOK_ENTITY, TOK_IDENTIFIER, "(", TOK_EXTENDS, TOK_LEFT_PAREN}, no_prim_args, {TOK_RIGHT_PAREN, ")?", TOK_COLON}});
    const signature entity_body_data =
        combine({{TOK_DATA, TOK_COLON, "("}, anytoken, {")*", TOK_IDENTIFIER, "(", TOK_COMMA, TOK_IDENTIFIER, ")*", TOK_SEMICOLON}});
    const signature entity_body_func =
        combine({{TOK_FUNC, TOK_COLON, "("}, anytoken, {")*", TOK_IDENTIFIER, "(", TOK_COMMA, TOK_IDENTIFIER, ")*", TOK_SEMICOLON}});
    const signature entity_body_link = combine({reference, {TOK_ARROW}, reference, {TOK_SEMICOLON}});
    const signature entity_body_links =
        combine({{TOK_LINK, TOK_COLON, "("}, anytoken, {")*("}, entity_body_link, {"("}, anytoken, {")*)+"}});
    ...
    const signature entity_body = combine({{"("}, entity_body_data, {")?("}, anytoken, {")*("}, entity_body_func, {")?("}, anytoken,
        {")*("}, entity_body_links, {")?("}, anytoken, {")*"}, entity_body_constructor});
    ...
    const signature par_for_loop = combine({{TOK_PAR_FOR, "((", TOK_UNDERSCORE, ")|(", TOK_IDENTIFIER, "))", TOK_COMMA, "((",
                                                TOK_UNDERSCORE, ")|(", TOK_IDENTIFIER, "))", TOK_IN},
        match_until_signature({TOK_COLON})});
