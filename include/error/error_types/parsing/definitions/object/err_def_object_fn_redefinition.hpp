...
class ErrDefObjectFnRedefinition : public BaseError {
  public:
    ErrDefObjectFnRedefinition(       //
    ...
    std::string to_string() const override {
        ...
        oss << BaseError::to_string() << "├─ Redefinition of function: " << YELLOW << original->name << DEFAULT
            << " from func module defined at " << GREEN << GREEN << cwd_relative(original->file_hash, original->line, original->column)
            << DEFAULT << "\n";
        oss << "├─ The explicit signatures of the function from the func-module matches the newly defined object function and creates a collision\n";
