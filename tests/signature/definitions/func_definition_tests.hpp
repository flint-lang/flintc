...
namespace {
    ...
    TestResult test_match_func_definition_normal() {
        ...
        bool result = Matcher::tokens_match({tokens.begin(), tokens.end()}, Matcher::func_definition);
...
namespace {
    ...
    TestResult test_contain_func_definition_normal() {
        ...
        bool result = Matcher::tokens_contain({tokens.begin(), tokens.end()}, Matcher::func_definition);
        test_result.ok_or_not(result);
        if (!result) {
            test_result.increment();
        }
...
namespace {
    ...
    TestResult test_extract_func_definition_normal() {
        ...
        token_list tokens = create_token_vector({//
            TOK_INDENT, TOK_FUNC, TOK_IDENTIFIER, TOK_COLON, TOK_EOL});
        std::vector<uint2> result_vec = Matcher::get_match_ranges({tokens.begin(), tokens.end()}, Matcher::func_definition);
