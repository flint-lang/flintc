...
namespace {
    ...
    TestResult test_match_type_int() {
        ...
        token_list tokens = create_token_vector({//
            TOK_I32});
        bool result = Matcher::tokens_match(tokens, Matcher::type);
...
namespace {
    ...
    TestResult test_contain_type_int() {
        ...
        bool result = Matcher::tokens_contain(tokens, Matcher::type);
        test_result.ok_or_not(result);
        if (!result) {
            test_result.increment();
        }
...
namespace {
    ...
    TestResult test_extract_type_int() {
        ...
        std::vector<uint2> result_vec = Matcher::get_match_ranges(tokens, Matcher::type);
