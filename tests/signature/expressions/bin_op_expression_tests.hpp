...
namespace {
    ...
    TestResult test_match_bin_op_expr_vars_square_int() {
        ...
        token_list tokens = create_token_vector({//
            TOK_INT_VALUE, TOK_POW, TOK_INT_VALUE});
        bool result = Matcher::tokens_match({tokens.begin(), tokens.end()}, Matcher::bin_op_expr);
...
namespace {
    ...
    TestResult test_contain_bin_op_expr_vars_square_int() {
        ...
        token_list tokens = create_token_vector({//
            TOK_IDENTIFIER, TOK_EQUAL, TOK_INT_VALUE, TOK_POW, TOK_INT_VALUE, TOK_SEMICOLON});
        bool result = Matcher::tokens_contain({tokens.begin(), tokens.end()}, Matcher::bin_op_expr);
        test_result.ok_or_not(result);
        if (!result) {
            test_result.increment();
        }
...
namespace {
    ...
    TestResult test_extract_bin_op_expr_vars_square_int() {
        ...
        token_list tokens = create_token_vector({//
            TOK_INT_VALUE, TOK_POW, TOK_INT_VALUE, TOK_SEMICOLON});
        std::vector<uint2> result_vec = Matcher::get_match_ranges({tokens.begin(), tokens.end()}, Matcher::bin_op_expr);
