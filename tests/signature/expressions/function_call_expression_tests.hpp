...
namespace {
    ...
    TestResult test_match_function_call_0arg() {
        ...
        bool result = Matcher::tokens_match({tokens.begin(), tokens.end()}, Matcher::function_call);
...
namespace {
    ...
    TestResult test_contain_function_call_0arg() {
        ...
        bool result = Matcher::tokens_contain({tokens.begin(), tokens.end()}, Matcher::function_call);
        test_result.ok_or_not(result);
        if (!result) {
            test_result.increment();
        }
...
namespace {
    ...
    TestResult test_extract_function_call_0arg() {
        ...
        token_list tokens = create_token_vector({//
            TOK_EQUAL, TOK_IDENTIFIER, TOK_LEFT_PAREN, TOK_RIGHT_PAREN, TOK_SEMICOLON});
        std::vector<uint2> result_vec = Matcher::get_match_ranges({tokens.begin(), tokens.end()}, Matcher::function_call);
