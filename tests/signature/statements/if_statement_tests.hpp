...
namespace {
    ...
    TestResult test_match_if_true() {
        ...
        bool result = Signature::tokens_match(tokens, Signature::if_statement);
...
namespace {
    ...
    TestResult test_contain_if_true() {
        ...
        bool result = Signature::tokens_contain(tokens, Signature::if_statement);
        test_result.ok_or_not(result);
        if (!result) {
            test_result.increment();
        }
...
namespace {
    ...
    TestResult test_extract_if_true() {
        ...
        token_list tokens = create_token_vector({//
            TOK_INDENT, TOK_IF, TOK_TRUE, TOK_COLON, TOK_EOL});
        std::vector<uint2> result_vec = Signature::get_match_ranges(tokens, Signature::if_statement);
