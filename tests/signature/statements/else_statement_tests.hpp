...
namespace {
    TestResult test_match_else() {
        ...
        token_list tokens = create_token_vector({//
            TOK_ELSE, TOK_COLON});
        bool result = Signature::tokens_match(tokens, Signature::else_statement);
...
namespace {
    TestResult test_contain_else() {
        ...
        bool result = Signature::tokens_contain(tokens, Signature::else_statement);
        test_result.ok_or_not(result);
        if (!result) {
            test_result.increment();
        }
...
namespace {
    TestResult test_extract_else() {
        ...
        token_list tokens = create_token_vector({//
            TOK_INDENT, TOK_ELSE, TOK_COLON, TOK_EOL});
        std::vector<uint2> result_vec = Signature::get_match_ranges(tokens, Signature::else_statement);
