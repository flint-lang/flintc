#include "parser/type/type.hpp"

#include "lexer/lexer_utils.hpp"
#include "parser/type/array_type.hpp"
#include "parser/type/opaque_type.hpp"
#include "parser/type/optional_type.hpp"
#include "parser/type/pointer_type.hpp"
#include "parser/type/primitive_type.hpp"
#include "parser/type/vector_type.hpp"

#include <mutex>
#include <optional>
#include <string>

uint32_t Type::get_id() const {
    return get_hash().get_type_id_from_str(to_string());
}

bool Type::is_reference() const {
    const auto variation = get_variation();
    if (variation == Variation::ENUM || variation == Variation::OPAQUE) {
        return false;
    }
    const std::string type_str = to_string();
    if (type_str == "str") {
        return true;
    }
    return primitives.find(type_str) == primitives.end();
}

void Type::init_types() {
    std::shared_ptr<Type> u8_type = get_primitive_type("u8");
    std::shared_ptr<Type> i8_type = get_primitive_type("i8");
    std::shared_ptr<Type> u16_type = get_primitive_type("u16");
    std::shared_ptr<Type> i16_type = get_primitive_type("i16");
    std::shared_ptr<Type> u32_type = get_primitive_type("u32");
    std::shared_ptr<Type> i32_type = get_primitive_type("i32");
    std::shared_ptr<Type> u64_type = get_primitive_type("u64");
    std::shared_ptr<Type> i64_type = get_primitive_type("i64");
    std::shared_ptr<Type> f32_type = get_primitive_type("f32");
    std::shared_ptr<Type> f64_type = get_primitive_type("f64");

    std::shared_ptr<Type> bool_type = get_primitive_type("bool");
    std::shared_ptr<Type> str_type = get_primitive_type("str");
    get_primitive_type("type.flint.str.lit");
    // Type for unresolved default-expressions `_`
    get_primitive_type("type.flint.default");
    std::shared_ptr<Type> void_type = get_primitive_type("void");
    add_type(std::make_shared<OptionalType>(void_type));
    add_type(std::make_shared<PointerType>(void_type));
    add_type(std::make_shared<OpaqueType>());
    get_primitive_type("anyerror");
    get_primitive_type("int");
    get_primitive_type("float");

    add_type(std::make_shared<VectorType>(bool_type, 8));

    add_type(std::make_shared<VectorType>(u8_type, 2));
    add_type(std::make_shared<VectorType>(u8_type, 3));
    add_type(std::make_shared<VectorType>(u8_type, 4));
    add_type(std::make_shared<VectorType>(u8_type, 8));

    add_type(std::make_shared<VectorType>(i8_type, 2));
    add_type(std::make_shared<VectorType>(i8_type, 3));
    add_type(std::make_shared<VectorType>(i8_type, 4));
    add_type(std::make_shared<VectorType>(i8_type, 8));

    add_type(std::make_shared<VectorType>(u16_type, 2));
    add_type(std::make_shared<VectorType>(u16_type, 3));
    add_type(std::make_shared<VectorType>(u16_type, 4));
    add_type(std::make_shared<VectorType>(u16_type, 8));

    add_type(std::make_shared<VectorType>(i16_type, 2));
    add_type(std::make_shared<VectorType>(i16_type, 3));
    add_type(std::make_shared<VectorType>(i16_type, 4));
    add_type(std::make_shared<VectorType>(i16_type, 8));

    add_type(std::make_shared<VectorType>(u32_type, 2));
    add_type(std::make_shared<VectorType>(u32_type, 3));
    add_type(std::make_shared<VectorType>(u32_type, 4));
    add_type(std::make_shared<VectorType>(u32_type, 8));

    add_type(std::make_shared<VectorType>(i32_type, 2));
    add_type(std::make_shared<VectorType>(i32_type, 3));
    add_type(std::make_shared<VectorType>(i32_type, 4));
    add_type(std::make_shared<VectorType>(i32_type, 8));

    add_type(std::make_shared<VectorType>(u64_type, 2));
    add_type(std::make_shared<VectorType>(u64_type, 3));
    add_type(std::make_shared<VectorType>(u64_type, 4));

    add_type(std::make_shared<VectorType>(i64_type, 2));
    add_type(std::make_shared<VectorType>(i64_type, 3));
    add_type(std::make_shared<VectorType>(i64_type, 4));

    add_type(std::make_shared<VectorType>(f32_type, 2));
    add_type(std::make_shared<VectorType>(f32_type, 3));
    add_type(std::make_shared<VectorType>(f32_type, 4));
    add_type(std::make_shared<VectorType>(f32_type, 8));

    add_type(std::make_shared<VectorType>(f64_type, 2));
    add_type(std::make_shared<VectorType>(f64_type, 3));
    add_type(std::make_shared<VectorType>(f64_type, 4));

    add_type(std::make_shared<ArrayType>(1, str_type, std::nullopt));
}

void Type::clear_types() {
    types.clear();
}

bool Type::add_type(const std::shared_ptr<Type> &type_to_add) {
    std::unique_lock<std::shared_mutex> lock(types_mutex);
    return types.emplace(type_to_add->to_string(), type_to_add).second;
}

std::shared_ptr<Type> Type::get_primitive_type(const std::string &type_str) {
    // Check if string is empty
    ASSERT(!type_str.empty(), "Identifier cannot be empty");
    // Check if the given type already exists in the types map
    {
        std::shared_lock<std::shared_mutex> shared_lock(types_mutex);
        if (types.find(type_str) != types.end()) {
            return types.at(type_str);
        }
    }
    // If the types map does not contain the type yet, lock the map to make it non-accessible while we modify it
    std::unique_lock<std::shared_mutex> lock(types_mutex);
    if (types.find(type_str) != types.end()) {
        // Another thread might already have added the type
        return types.at(type_str);
    }
    // Create a primitive type from the type_str
    types[type_str] = std::make_shared<PrimitiveType>(type_str);
    return types.at(type_str);
}

std::optional<std::shared_ptr<Type>> Type::get_type_from_str(const std::string &type_str) {
    std::shared_lock<std::shared_mutex> shared_lock(types_mutex);
    if (types.find(type_str) == types.end()) {
        return std::nullopt;
    }
    return types.at(type_str);
}
