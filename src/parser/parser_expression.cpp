...
std::optional<std::unique_ptr<ExpressionNode>> Parser::create_string_interpolation( //
    ...
    for (auto it = ranges.begin(); it != ranges.end(); ++it) {
        ...
        // Casting the expression content to a str type happens in the analyzer
    ...
    for (auto &elem : interpol_content) {
        ...
        if (std::holds_alternative<std::unique_ptr<ExpressionNode>>(elem)) {
            ...
            // Check if it's a type.flint.str.lit literal
    ...
    // If the result is a single string literal, return it directly
    if (optimized_content.size() == 1) {
        if (std::holds_alternative<std::unique_ptr<ExpressionNode>>(optimized_content[0])) {
            ...
            if (lit->type->to_string() == "type.flint.str.lit") {
                return std::make_unique<LiteralNode>(std::move(*lit));
            }
        }
    }
...
std::optional<std::unique_ptr<ExpressionNode>> Parser::create_initializer( //
        ...
        switch (type_token->type->get_variation()) {
            ...
            case Type::Variation::TUPLE: {
                ...
                for (size_t i = 0; i < tuple_type->types.size(); i++) {
                    const std::string tuple_field_name = "$" + std::to_string(i);
                    possible_fields.emplace_back(tuple_field_name, tuple_type->types.at(i));
                    if (tuple_field_name == field_name) {
                        field_found = true;
                        field_type = tuple_type->types.at(i);
                        break;
                    }
                }
                if (!field_found) {
    ...
    switch (type_token->type->get_variation()) {
        ...
        case Type::Variation::TUPLE: {
            const auto *tuple_type = type_token->type->as<TupleType>();
            if (tuple_type->types.size() == fields.size()) {
                break;
            }
            for (size_t i = 0; i < tuple_type->types.size(); i++) {
                const auto &type = tuple_type->types.at(i);
...
std::optional<std::unique_ptr<ExpressionNode>> Parser::create_pivot_expression( //
    ...
    // Match the two operands of the binary operation against each other, coercing them in place. This is performed at parse time so that
