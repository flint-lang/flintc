...
void Generator::Module::Print::generate_print_function( //
    ...
    llvm::Function *const print_function = llvm::Function::Create( //
        print_type,                                                //
    ...
    llvm::BasicBlock *const block = llvm::BasicBlock::Create(context, "entry", print_function);
    ...
    llvm::Value *const format_str = IR::generate_const_string(module, format);
    builder->CreateCall(c_functions.at(PRINTF), {format_str, arg});
...
void Generator::Module::Print::generate_print_str_lit_function( //
    ...
    // Create print function type
    llvm::FunctionType *const print_str_lit_type = llvm::FunctionType::get( //
        llvm::Type::getVoidTy(context),                                     // Return type: void
        {str_lit_type},                                                     // Argument: char* literal
        false                                                               // No vaargs
    );
    // Create the print_str_lit function
    llvm::Function *const print_str_lit_function = llvm::Function::Create( //
        print_str_lit_type,                                                //
        llvm::Function::ExternalLinkage,                                   //
        prefix + "str_lit",                                                //
        module                                                             //
    ...
    llvm::BasicBlock *const block = llvm::BasicBlock::Create(context, "entry", print_str_lit_function);

    // Set insert point to the current block
    builder->SetInsertPoint(block);

    // Set the argument name of the function
    llvm::Value *const arg_literal = print_str_lit_function->getArg(0);
...
void Generator::Module::Print::generate_print_str_var_function( //
    ...
    // Create print function type
    llvm::FunctionType *const print_str_type = llvm::FunctionType::get( //
        llvm::Type::getVoidTy(context),                                 // Return Type: void
        {PTR_TY},                                                       // Argument: str* string
        false                                                           // No varargs
    );
    // Create the print_int function
    llvm::Function *const print_str_function = llvm::Function::Create( //
        print_str_type,                                                //
        llvm::Function::ExternalLinkage,                               //
        prefix + "str",                                                //
        module                                                         //
    ...
    llvm::BasicBlock *const block = llvm::BasicBlock::Create(context, "entry", print_str_function);

    // Set insert point to the current block
    builder->SetInsertPoint(block);

    // Convert it to fit the correct format printf expects
    llvm::Value *const arg_string = print_str_function->getArg(0);
    arg_string->setName("string");

    llvm::Value *const str_len_ptr = builder->CreateStructGEP(str_type, arg_string, 0, "str_len_ptr");
    llvm::Value *const str_len = IR::aligned_load(*builder, llvm::Type::getInt64Ty(context), str_len_ptr, "str_len");
    llvm::Value *const str_len_val = TypeCast::uN_to_iN_trunc(*builder, str_len, 32);

    llvm::Value *const str_val_ptr = builder->CreateStructGEP(str_type, arg_string, 1, "str_val_ptr");

    // Call printf with format string and argument
    llvm::Value *const format_str = IR::generate_const_string(module, "%.*s");
...
void Generator::Module::Print::generate_print_bool_function( //
    ...
    llvm::BasicBlock *const entry_block = llvm::BasicBlock::Create(context, "entry", print_function);
    llvm::BasicBlock *const true_block = llvm::BasicBlock::Create(context, "bool_true", print_function);
    llvm::BasicBlock *const false_block = llvm::BasicBlock::Create(context, "bool_false", print_function);
    llvm::BasicBlock *const merge_block = llvm::BasicBlock::Create(context, "merge", print_function);

    llvm::Argument *const arg = print_function->getArg(0);
    ...
    llvm::Value *const str_true = IR::generate_const_string(module, "true");
    builder->CreateCall(c_functions.at(PRINTF), {format_str, str_true});
    builder->CreateBr(merge_block);

    // False block
    builder->SetInsertPoint(false_block);
    llvm::Value *const str_false = IR::generate_const_string(module, "false");
